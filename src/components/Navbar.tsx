...
const routes: { title: string; href: string }[] = [
  ...
  const Navbar: React.FC = () => {
    ...
    return (   
      <div className="relative flex items-center justify-between h-32 w-full bg-gradient-to-t from-transparent to-black lg:pr-14 md:pr-10 sm:pr-2 lg:pl-8 py-5">   
        <div className="flex w-full justify-between m-6">
          ...
          {routes.map((route, index) => (
            <Link
              ...
              href={route.href}
              className={
                "hover:text-[#59BC89] transition text-white items-center inline-flex lg:px-5 md:px-3 sm:px-1.5 lg:text-base md:text-sm sm:text-xs"
              }
            >
              {route.title}
            </Link>
